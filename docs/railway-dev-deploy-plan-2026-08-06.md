...
- Two credentials:
  - **owner/migration** credential — used only by a separate, private,
    ...
    only there). Applies `pkg/db/migrations` through `00012` and the marker
...
- Schema acceptance: `schema_migrations` must read exactly `12|false` before
...
2. Provision §4 (database, two roles, migration job run, `12|false`
   verified read-only).
...
`schema_migrations` reads `12|false`. It never sends authenticated API
